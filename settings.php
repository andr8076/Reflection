...
<body class="automation-page bg-light text-dark container-fluid px-3 px-md-4 py-4">
    <header class="hero row g-3 align-items-stretch mb-4 compact-hero">
        <div class="hero-main col-12 col-lg bg-white border rounded-4 shadow-sm p-4">
            ...
            <h1>Settings</h1>
            <p class="lede text-secondary fs-5 mb-3">Farm policy, ESS, Wake-on-LAN, retention, crash-loop protection, and configured farm machines.</p>
        ...
        <aside class="version-card col-12 col-lg-4 bg-white border rounded-4 shadow-sm p-4 d-flex flex-column gap-2">
    ...
    <main class="settings-layout row g-4 align-items-start">
        <section class="panel bg-white border rounded-4 shadow-sm p-4 mb-4 settings-page-panel col-12 col-xl-8">
            <div class="panel-head d-flex justify-content-between align-items-start gap-3 flex-wrap mb-3">
                <div>
                    <p class="eyebrow text-primary text-uppercase fw-bold small mb-1">Master</p>
                    <h2>Farm settings</h2>
                </div>
                <form method="post" class="inline-form d-inline-flex gap-2 align-items-center">
                    <input type="hidden" name="settings_action" value="maintenance">
                    <button type="submit" class="ghost-button btn btn-outline-primary small-button btn-sm">Run maintenance now</button>
                </form>
            </div>
            <form method="post" class="settings-form d-grid gap-3">
                ...
                <div class="settings-section-box bg-light border rounded-3 p-3">
                    ...
                    <div class="settings-grid row g-3">
                        ...
                        <p class="api-note text-secondary small">When enabled, the master advertises its Git commit. Workers that see a different commit self-update to that exact commit before accepting work. If the master cannot advertise a commit, workers ignore the version check.</p>
                        <label>
                            Failed task behavior
                            <select class="form-select" name="failure_strategy">
                        ...
                        <label>
                            ...
                            <input type="number" name="max_retries" min="0" value="<?= (int) ($settings['max_retries'] ?? 0) ?>" class="form-control">
                        </label>
                        <label>
                            Lost/crashed job behavior
                            <select class="form-select" name="stale_job_strategy">
                ...
                <div class="settings-section-box bg-light border rounded-3 p-3">
                    ...
                    <div class="settings-grid row g-3">
                        ...
                        <label>
                            ...
                            <input type="number" name="crash_loop_lost_attempts" min="1" value="<?= (int) ($settings['crash_loop_lost_attempts'] ?? 2) ?>" class="form-control">
                        </label>
                        <label>
                            Distinct workers before block
                            <input type="number" name="crash_loop_distinct_workers" min="1" value="<?= (int) ($settings['crash_loop_distinct_workers'] ?? 1) ?>" class="form-control">
                ...
                <div class="settings-section-box bg-light border rounded-3 p-3">
                    ...
                    <div class="settings-grid row g-3">
                        <label>
                            ...
                            <input type="number" name="ess_min_soc_percent" min="0" max="100" value="<?= (int) ($settings['ess_min_soc_percent'] ?? 20) ?>" class="form-control">
                        </label>
                        <label>
                            ESS SOC URL
                        ...
                        <label>
                            ...
                            <input type="number" name="auto_wake_cooldown_seconds" min="0" value="<?= (int) ($settings['auto_wake_cooldown_seconds'] ?? 300) ?>" class="form-control">
                        </label>
                        <label>
                            Max demand wakes per run
                            <input type="number" name="auto_wake_max_targets_per_run" min="0" value="<?= (int) ($settings['auto_wake_max_targets_per_run'] ?? 20) ?>" class="form-control">
                        </label>
                        <label>
                            WOL broadcast address
                            <input name="wake_broadcast_address" value="<?= reflection_h($settings['wake_broadcast_address'] ?? '255.255.255.255') ?>" class="form-control" placeholder="255.255.255.255">
                        ...
                        <label>
                            WOL UDP port
                            <input type="number" name="wake_udp_port" min="1" max="65535" value="<?= (int) ($settings['wake_udp_port'] ?? 9) ?>" class="form-control">
                        </label>
                        <label>
                            Wake-on-LAN delivery method
                            <select class="form-select" name="wake_dispatch_mode">
                    ...
                    <div class="ess-status-box alert alert-info">
                ...
                <div class="settings-section-box bg-light border rounded-3 p-3">
                    ...
                    <div class="settings-grid row g-3">
                        <label>
                            ...
                            <input type="number" name="job_history_keep_completed" min="0" value="<?= (int) ($settings['job_history_keep_completed'] ?? 500) ?>" class="form-control">
                        </label>
                        <label>
                            Event log lines to keep
                            <input type="number" name="event_log_keep_lines" min="0" value="<?= (int) ($settings['event_log_keep_lines'] ?? 1000) ?>" class="form-control">
                        </label>
                        <label>
                            File-history paths to keep
                            <input type="number" name="file_history_keep_paths" min="0" value="<?= (int) ($settings['file_history_keep_paths'] ?? 500) ?>" class="form-control">
                        </label>
                        <label>
                            Entries per file-history path
                            <input type="number" name="file_history_keep_entries_per_path" min="0" value="<?= (int) ($settings['file_history_keep_entries_per_path'] ?? 10) ?>" class="form-control">
                        </label>
                        <label>
                            Archived job lines to keep
                            <input type="number" name="job_archive_keep_lines" min="0" value="<?= (int) ($settings['job_archive_keep_lines'] ?? 5000) ?>" class="form-control">
                        </label>
                        <label>
                            Worker temp cleanup age hours
                            <input type="number" name="worker_temp_max_age_hours" min="1" value="<?= (int) ($settings['worker_temp_max_age_hours'] ?? 24) ?>" class="form-control">
                        </label>
                        <label>
                            Remote quarantine keep days
                            <input type="number" name="quarantine_keep_days" min="1" value="<?= (int) ($settings['quarantine_keep_days'] ?? 14) ?>" class="form-control">
                        </label>
                        <label>
                            Remote quarantine max GB per folder
                            <input type="number" name="quarantine_max_gb" min="0" step="0.1" value="<?= htmlspecialchars((string) ($settings['quarantine_max_gb'] ?? 100)) ?>" class="form-control">
                ...
                <div class="settings-section-box bg-light border rounded-3 p-3">
                    <div class="panel-head d-flex justify-content-between align-items-start gap-3 flex-wrap mb-3 inline-panel-head">
                        <div>
                            <h3>Farm computers and Wake-on-LAN</h3>
                            <p class="api-note text-secondary small">Add each farm computer once. Per-computer minimum SOC is the ESS percentage that machine may run down to. Leave it blank to use the global fallback above.</p>
                        </div>
                        <button type="button" class="ghost-button btn btn-outline-primary small-button btn-sm" data-add-machine-row>Add computer</button>
                    </div>
                    <div class="table-wrap table-responsive machine-editor mt-3" data-machine-editor>
                        <table class="machine-editor-table table table-sm align-middle table-hover mb-0">
                            <thead>
                                <tr>
                                    <th>Computer ID</th>
                                    <th>MAC address</th>
                            ...
                            <?php foreach ($machineRows as $machineIndex => $machine): ?>
                                ...
                                <tr>
                                    <td><input name="machine_pc_id[<?= (int) $machineIndex ?>]" class="form-control" value="<?= reflection_h($machine['pc_id'] ?? '') ?>" placeholder="farm1"></td>
                ...
                <div class="settings-section-box bg-light border rounded-3 p-3">
                    <div class="panel-head d-flex justify-content-between align-items-start gap-3 flex-wrap mb-3 inline-panel-head">
                        <div>
                            ...
                            <p class="api-note text-secondary small">Workers report <code>.reflection_quarantine</code> folders when they safely replace an existing remote output. Manual delete queues a worker control job that empties that tracked folder.</p>
                        </div>
                    </div>
                    <?php if ($quarantineLocations === []): ?>
                        <p class="empty text-secondary text-center py-4">No remote quarantine folders have been reported yet.</p>
                    <?php else: ?>
                        <div class="table-wrap table-responsive">
                            <table class="compact-table quarantine-table table table-sm table-hover align-middle mb-0">
                                <thead>
                                    <tr>
                                        <th>Folder</th>
                                        <th>Last seen</th>
                                ...
                                <?php foreach ($quarantineLocations as $location): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <form method="post" class="inline-form d-inline-flex gap-2 align-items-center" onsubmit="return confirm('Queue a worker job to delete all files in this quarantine folder?');">
                                                ...
                                                <button type="submit" class="danger-button btn btn-outline-danger small-button btn-sm">Delete contents</button>
                                            </form>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
        ...
        <aside class="panel bg-white border rounded-4 shadow-sm p-4 mb-4 automation-sidebar col-12 col-xl-4">
            <div class="panel-head d-flex justify-content-between align-items-start gap-3 flex-wrap mb-3"><div><p class="eyebrow text-primary text-uppercase fw-bold small mb-1">Live checks</p><h2>Actions</h2></div></div>
            <form method="post" class="form-block d-grid gap-2 mb-3 boxed-form-block bg-light border rounded-3 p-3">
                <input type="hidden" name="settings_action" value="refresh_ess">
                <h3>ESS SOC parser</h3>
                <p class="api-note text-secondary small">Fetch the configured ESS endpoint now and update the read-only SOC status.</p>
                <button type="submit" class="ghost-button btn btn-outline-primary">Check ESS now</button>
            </form>
            <div class="form-block d-grid gap-2 mb-3 boxed-form-block bg-light border rounded-3 p-3">
                <h3>Related pages</h3>
                <p><a class="ghost-button btn btn-outline-primary" href="logs.php">Open Logs</a></p>
                <p><a class="ghost-button btn btn-outline-primary" href="system_checks.php">Open System checks</a></p>
                <p><a class="ghost-button btn btn-outline-primary" href="storage_servers.php">Open Storage servers</a></p>
            </div>
        </aside>
    </main>
