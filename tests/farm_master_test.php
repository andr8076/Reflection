...
assertSameValue(false, $socStore->effectiveSettings()['ess_charging'], 'ESS JSON charging=false should be stored.');
assertSameValue(0, $socStore->allowedActiveWorkers(), 'Charging override must not bypass SOC limits when charging is false.');
file_put_contents($socEndpointPath, '{"soc":0.393197836719441,"charging":true}');
assertSameValue(39, $socStore->refreshEssSocFromConfiguredEndpoint(), 'ESS JSON SOC should still parse while charging.');
assertSameValue(true, $socStore->effectiveSettings()['ess_charging'], 'ESS JSON charging=true should be stored.');
assertSameValue(PHP_INT_MAX, $socStore->allowedActiveWorkers(), 'Charging override should bypass SOC limits while the ESS reports charging.');
$socStore->updateSettings(['ess_charging_override_enabled' => false]);
assertSameValue(0, $socStore->allowedActiveWorkers(), 'Disabling charging override should make the minimum SOC apply again.');
$socStore->updateSettings(['ess_min_soc_percent' => 20, 'ess_charging_override_enabled' => true]);
...
assertSameValue(2, count($wakePlan['targets']), 'Demand wake should wake offline machines whose minimum ESS SOC is met.');
...
assertSameValue('node-wake-3', $wakePlan['targets'][1]['pc_id'], 'Demand wake should not treat minimum ESS SOC as a cumulative battery budget.');
...
assertSameValue(4, $marginStore->allowedActiveWorkers(), 'SOC worker allowance should count all configured workers whose minimum ESS SOC is met, including workers without Wake-on-LAN.');
$marginTargets = $marginStore->wakeTargetsForCurrentSoc(true, 900);
assertSameValue(2, count($marginTargets), 'Manual wake target lookup should exclude online workers and keep offline workers whose minimum ESS SOC is met.');
...
$priorityJobA = $priorityStore->createJob('dummy_task', 'incoming/layer-a.dat', 'outputs/layer-a.txt', false);
$priorityHighResponse = reflection_handle_farm_api([
    'action' => 'request_task',
    'version' => 'test-version',
    'pc_id' => 'layer1-node',
...
assertSameValue(true, $coreUpdateResponse['update_allowed'], 'Version-follow updates should not wait for shutdown/update layers.');
assertSameValue('test-version', $coreUpdateResponse['target_version'], 'The master should tell the worker exactly which commit to install.');
...
assertSameValue('shutdown_confirmed', $confirmShutdownResponse['status'], 'Workers should be able to confirm shutdown orders explicitly.');
